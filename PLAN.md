...
- Интеграция `bundle` с реальными `walker` и `images` после готовности параллельных задач, подтяжка метаданных из `config`/CLI.
...
- [ ] Статическая типизация (базовый mypy, строгие секции точечно)
