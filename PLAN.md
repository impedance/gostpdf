...
- Готовы и покрыты юнит-тестами: `walker.py`, `images.py`, `bundle.py`, `config.py`, `pandoc_runner.py`, `reporting.py`; базовый `mypy` настроен (`walker` в strict).
- Не реализованы: `pipeline.py`, `cli.py`; нет сквозного пайплайна. Бандл мержит метаданные и умеет запись на диск, интеграция в пайплайн/CLI впереди.
...
- **T3 — Пайплайн (подзадачи, берем независимо)**:
  1. Валидация/мердж параметров (config + overrides, подготовка путей).
  2. Обвязка `walker` (сбор Markdown и предупреждений).
  3. Обвязка `bundle` (сбор и запись бандла).
  4. Обвязка `pandoc_runner` (вызов рендера PDF).
  5. Агрегация результатов/предупреждений.
...
- [ ] T3: Пайплайн (оркестратор, мерж override, запись bundle, вызов runner; агрегация предупреждений готова)
