...
- Готовы и покрыты юнит-тестами: `walker.py`, `images.py`, `bundle.py`, `config.py`, `pandoc_runner.py`, `reporting.py`, вспомогательные части `pipeline.py` (валидация параметров, сбор markdown, сборка/запись бандла, вызов Pandoc, агрегация предупреждений), `cli.py`; базовый `mypy` настроен (`walker` в strict).
- Не реализованы: нет сквозного интеграционного теста пайплайна.
