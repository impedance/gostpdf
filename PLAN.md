...
- Не реализованы: `pandoc_runner.py`, `cli.py`; нет сквозного пайплайна. Бандл пока использует только `DEFAULT_BUNDLE_METADATA`, метаданные из `config`/CLI не подмешиваются.
...
- [ ] T1: Бандл — метаданные/запись/переписывание через images
